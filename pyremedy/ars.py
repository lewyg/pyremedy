...
class ARS(object):
    ...
    def __init__(self, server, user, password, port=0, rpc_program_number=0):
        ...
        verify_user_status = self.arlib.ARVerifyUser(
            # ARControlStruct *control: the control record
            byref(self.control),
            # ARBoolean *adminFlag: user is a member of the Administrator group
            None,
            # ARBoolean *subAdminFlag: user is a member of the Subadministrator group
            None,
            # ARBoolean *customFlag: user is a member of the Customize group
            None,
            # (return) ARStatusList *status: notes, warnings or errors
            # generated by the operation
            byref(self.status)
        ...
        if verify_user_status >= arh.AR_RETURN_ERROR:
            self._update_errors()
            self.arlib.FreeARStatusList(byref(self.status), arh.FALSE)
